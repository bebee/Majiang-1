...
module TipsManager {
	...
	export function showTips(e): void {
		...
		if (this._tips == null) {
			...
			if (!LayerManager.gameLayer().effectLayer.contains(this._tips)) {
				LayerManager.gameLayer().effectLayer.addChild(this._tips);
			}
	...
	export function removeTips(e): void {
		...
		if (my._tips != null) {
			var onComplete: Function = function () {
				if (LayerManager.gameLayer().effectLayer.contains(my._tips)) {
					LayerManager.gameLayer().effectLayer.removeChild(my._tips);

					my._tips = null;
				}
			};

			egret.Tween.get(my._tips).to({alpha: 0}, 300).call(onComplete, my);
		}
	}
}