...
class TextEffect {
    ...
    public static showTipsDownToUp(str: string = "", isWarning: boolean = false): void {
        ...
        effectTips.x = acekit.width / 2 - effectTips.width / 2;
        effectTips.stroke = 2;
        effectTips.bold = true;
        effectTips.textAlign = egret.HorizontalAlign.CENTER;

        if (!acekit.contains(effectTips)) {
            acekit.addChild(effectTips);
        }

        var onComplete2: Function = function () {
            if (acekit.contains(effectTips)) {
                acekit.removeChild(effectTips);
                effectTips = null;
            }
        };
    ...
    public static showTipsLeftOrRight(str: string = "", isWarning: boolean = false, isFromeLeft: boolean = true): void {
        ...
        if (isFromeLeft) {
            egret.Tween.get(effectTips).to({
                x: acekit.width / 2 - effectTips.width / 2 - 50,
                alpha: 1
        ...
        egret.setTimeout(function () {
            if (isFromeLeft) {
                egret.Tween.get(effectTips).to({x: acekit.width}, 300, egret.Ease.sineIn);
            } else {
